...
void main() {
  test('permission-denied maps to PermissionFailure', () {
    expect(
      mapFirebaseException(_exception('permission-denied')),
  ...
  test('not-found maps to NotFoundFailure', () {
    expect(
      mapFirebaseException(_exception('not-found')),
  ...
  test('unavailable maps to NetworkFailure', () {
    expect(
      mapFirebaseException(_exception('unavailable')),
  ...
  test('network-request-failed maps to NetworkFailure', () {
    expect(
      mapFirebaseException(_exception('network-request-failed')),
      const NetworkFailure(),
    );
  });
  ...
  for (final code in [
    ...
    test('$code maps to InvalidCredentialsFailure', () {
      expect(
        mapFirebaseException(_exception(code)),
  ...
  test(
    'an unrecognized code with a message maps to UnknownFailure(message)',
    () {
      final failure = mapFirebaseException(
        _exception('some-other-code', message: 'weird error'),
      );
  ...
  test(
    'an unrecognized code with no message falls back to UnknownFailure(code)',
    () {
