...
void main() {
  ...
  group('happy paths (firebase_auth_mocks + fake_cloud_firestore)', () {
    test(
      'authStateChanges emits null when signed out, then the resolved AppUser',
      () async {
  ...
  group('failure paths (mocktail)', () {
    ...
    group('signIn', () {
      test('maps a FirebaseAuthException to a typed Failure', () async {
        ...
        final result = await FirebaseAuthDataSource(
          auth,
          firestore,
        ).signIn(email: 'x@example.com', password: 'bad');

        expect(result, const Left(InvalidCredentialsFailure()));
      });

      test(
        'maps a signed-in user with no role doc to PermissionFailure',
        () async {
          ...
          final result = await FirebaseAuthDataSource(
            auth,
            firestore,
          ).signIn(email: 'x@example.com', password: 'whatever');
      ...
      test(
        'maps a Firestore error during role lookup to UnknownFailure',
        () async {
          final credential = MockUserCredential();
          final user = MockFbUser();
          final collection = MockCollectionReference();
